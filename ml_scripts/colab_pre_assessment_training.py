...
# Load the dataset
try:
    df = pd.read_csv('synthetic_pre_assessment_data.csv')
except FileNotFoundError:
    print("ERROR: Please upload 'synthetic_pre_assessment_data.csv' to Colab first!")
    exit()

# Separate Features (X) and Target (y)
X = df[['age', 'gradeLevel', 'preTestScore']]
y = df['ModuleQuizDifficulty']
...
joblib.dump(le, 'pre_assessment_label_encoder.joblib')

# Trigger download automatically in Colab
print("Downloading files to your computer...")
files.download('best_pre_assessment_model.joblib')
files.download('pre_assessment_scaler.joblib')
files.download('pre_assessment_label_encoder.joblib')
